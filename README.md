...
基于 FastAPI、LangChain 与 RAGflow 的**智能保险指南与增强 RAG 系统**，支持 PC Web、管理看板、Gradio 演示与小程序复用 API。实现多轮对话、意图识别、问题改写、知识库检索（RAGflow）、答案生成与合规校验，以及医疗条款解析、保障重叠度透视、管理看板等核心功能。

> **架构说明**：[doc/系统架构/架构说明.md](doc/系统架构/架构说明.md)  
> **项目说明**：[doc/系统架构/项目说明.md](doc/系统架构/项目说明.md)  
> **全部场景整体架构**：[doc/系统架构/智保灵犀_全部场景整体架构.md](doc/系统架构/智保灵犀_全部场景整体架构.md)  
> **中间件与安装**：[README_MIDDLEWARE.md](README_MIDDLEWARE.md)  
> **详细技术实现**：[doc/系统架构/详细技术实现方案.md](doc/系统架构/详细技术实现方案.md)  
> **阿里云 ECS Docker 部署**：[doc/系统架构/Docker部署说明-阿里云ECS.md](doc/系统架构/Docker部署说明-阿里云ECS.md)（后续更新可执行 `./scripts/deploy.sh` 一键部署）  
> **运维小白一步一操**：[doc/系统架构/ECS服务器Docker部署指南-小白版.md](doc/系统架构/ECS服务器Docker部署指南-小白版.md)（MySQL/Redis/ES 已安装，按步骤部署）
...
- **流式回复**：`/api/chat/stream` 返回 SSE，答案逐 chunk 推送
- **双模型方案**：专业版（qwen-plus）/ 标准版（qwen-turbo），可按请求选择
- **医疗条款解析**：支持上传 PDF/Word/TXT 或粘贴文本，基于 RAGflow 创建临时知识库，多轮咨询时优先基于条款回答
- **保障重叠度透视镜**：多产品保障责任重叠度分析与缺口识别
- **管理看板**：近 N 天核心指标（交互总量、合规拦截、条款解析次数、活跃用户、意图分布）
- **合规与审计**：违规词屏蔽、交互日志与合规日志写入 MySQL
- **用户认证**：JWT 登录/注册，支持 `/api/auth/me`、对话历史等鉴权接口
- **对话上下文压缩**：支持 recent_only / similarity / hybrid 模式，控制上下文长度与 token 消耗
- **数据与检索**：MySQL（用户与日志）、Redis（对话/条款上下文）、RAGflow（主知识库 + 条款临时库）、ChromaDB（意图/改写规则）、Elasticsearch（可选）
- **前端**：PC Web 单页（`web/static/index.html`）、管理看板（`web/static/admin.html`）、Gradio 演示（`gradio_ui`），API 可复用于小程序
...
# 医疗条款解析（可选）
CLAUSE_PARSE_ENABLED=True
CLAUSE_UPLOAD_MAX_SIZE=10485760
CLAUSE_ALLOWED_EXT=pdf,doc,docx,txt

# 保障重叠度（可选）
COVERAGE_OVERLAP_ENABLED=True

# 对话上下文压缩（可选）
CONTEXT_COMPRESSION_ENABLED=True
CONTEXT_SELECTION_MODE=hybrid
```
...
**启动 API 服务（供 PC Web / 管理看板 / Gradio / 小程序调用）：**
...
- 管理看板：http://localhost:8000/static/admin.html（需登录）
...
│   │   ├── 项目说明.md
│   │   ├── 智保灵犀_全部场景整体架构.md
│   │   ├── Docker部署说明-阿里云ECS.md
│   │   ├── ECS服务器Docker部署指南-小白版.md
│   │   └── ...
│   ├── 产品需求/              # P0/P1/P2 产品技术方案
│   └── 保险知识/              # 知识库文档
├── config/                   # 配置层
...
├── routers/                  # API 路由
│   ├── auth.py               # 认证（注册/登录/me）
│   ├── chat.py               # 增强 RAG 对话（/api/chat、/api/chat/stream、history、context/restore）
│   ├── clause.py             # 医疗条款解析（上传/上下文/清除）
│   ├── admin.py              # 管理看板（/api/admin/dashboard）
│   ├── vector.py             # 向量库
│   ├── es.py                 # Elasticsearch
...
│   ├── clause_context.py     # 条款上下文管理（Redis）
│   ├── ragflow_dataset.py    # RAGflow 临时知识库（条款解析）
│   ├── coverage_overlap.py   # 保障重叠度计算
│   ├── context_compressor.py  # 对话上下文压缩
│   ├── ragflow_client.py     # RAGflow 检索客户端
│   ├── model_plan.py         # 专业版/标准版模型选择
│   └── ...
├── models/                   # ORM 模型（用户、交互日志、合规日志）
├── web/                      # PC Web 前端
│   └── static/               # index.html（主页）、admin.html（管理看板）
...
- **Redis**：存储多轮对话上下文与条款上下文，`/api/chat`、`/api/chat/clear`、`/api/clause/*` 依赖 Redis。
...
- **医疗条款解析**：`CLAUSE_PARSE_ENABLED` 控制开关，`CLAUSE_UPLOAD_MAX_SIZE`、`CLAUSE_ALLOWED_EXT` 限制上传。
- **保障重叠度**：`COVERAGE_OVERLAP_ENABLED` 启用后，`COVERAGE_OVERLAP_KB_IDS` 可配置多库（逗号分隔）。
- **对话上下文压缩**：`CONTEXT_COMPRESSION_ENABLED`、`CONTEXT_SELECTION_MODE`、`CONTEXT_MAX_TURNS` 等。
...
| 对话 | POST | `/api/chat` | 多轮对话（body: `user_id`, `query`，可选 `intent_mode`, `rewrite_mode`, `model_plan`, `session_id`, `clause_text`） |
| 对话 | POST | `/api/chat/stream` | 流式对话，返回 SSE |
| 对话 | POST | `/api/chat/clear` | 清除指定用户对话上下文 |
| 对话 | GET | `/api/chat/history` | 当前用户最近对话记录（需登录） |
| 对话 | GET | `/api/chat/history/{log_id}` | 单条记录详情（需登录） |
| 对话 | POST | `/api/chat/context/restore` | 从某条历史恢复上下文（含条款快照，需登录） |
| 条款 | POST | `/api/clause/upload` | 上传医疗条款文件 PDF/Word/TXT（需登录） |
| 条款 | GET | `/api/clause/context` | 获取当前会话条款上下文（需登录） |
| 条款 | POST | `/api/clause/clear` | 清除当前会话条款上下文（需登录） |
| 管理 | GET | `/api/admin/dashboard` | 管理看板：近 N 天核心指标（需登录） |
...
curl -X POST "http://localhost:8000/api/chat/stream" \
  -H "Content-Type: application/json" \
  -d '{"user_id": "user001", "query": "百万医疗能报多少？"}' \
  -N
```
...
更多示例见 [doc/系统架构/手工验收CheckList.md](doc/系统架构/手工验收CheckList.md)。
